...
func CheckNonEquivocatingCertificates(prevUC, newUC *UnicityCertificate) error {
	...
	// Missing previous UC is acceptable in two cases:
	// 1. New UC is the initial UC with state hash == nil. This is generated by
	//    root chain when shard is activated and thus shard node cannot have it in advance.
	// 2. New UC is the first UC with shard proposed state (meaning prev state hash == nil).
	//    Such UC is the first one stored in blockStore and thus prevUC will be nil for the
	//    first UC when restoring from blockStore.
	if prevUC == nil && (newUC.IsInitial() || newUC.InputRecord.PreviousHash == nil) {
		return nil
	}
	if prevUC == nil {
